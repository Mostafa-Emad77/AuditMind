...
│   ├── app/
│   │   ├── __init__.py
│   │   ├── main.py                    # FastAPI app, SSE routes, background audit execution
│   │   ├── config.py                  # Pydantic Settings (all env vars)
│   │   ├── dependencies.py            # Optional X-API-Key auth dependency
│   │   ├── agents/
...
│   │   │   ├── cross_checker/         # 4-phase contradiction detection engine
│   │   │   │   ├── agent.py           # Phase orchestration
│   │   │   │   ├── adjudication.py    # LLM-based pair/check adjudication
│   │   │   │   ├── dedup.py           # Topic bucketing + numeric deduplication
│   │   │   │   ├── gates.py           # Acceptance gates (confidence, evidence)
│   │   │   │   └── evidence.py        # Evidence helpers
│   │   │   ├── report_writer.py       # Structured report synthesis (Arabic/English)
│   │   │   └── qa_agent.py            # Conversational retrieval-grounded Q&A
│   │   ├── tools/
│   │   │   ├── hybrid_retriever.py    # Rule-based router: Qdrant + Neo4j merged results
│   │   │   ├── neo4j_tools.py         # Graph search + contradiction detection tools
│   │   │   ├── planner_tools.py       # Checklist generation LLM tool
│   │   │   └── financial.py           # Financial figure extraction/compare tools
│   │   ├── services/
...
│   │   │   ├── vector_store.py           # Qdrant client wrapper + embedding encoder
│   │   │   ├── graph_builder.py          # Neo4j CRUD + schema init + contradiction queries
│   │   │   ├── redis_store.py            # Async session/report/chat/triage persistence
│   │   │   └── reconciliation_payload.py # Contract/invoice/bank reconciliation snapshot
│   │   ├── models/
│   │   │   ├── schemas.py             # AuditReport, Finding, AuditSession, ChatRequest, etc.
│   │   │   └── state.py               # AuditState (LangGraph TypedDict)
│   │   └── utils/
│   │       ├── llm_factory.py         # OpenRouter / Google LLM selector (cached)
│   │       ├── llm_json.py            # Consolidated JSON extraction from LLM responses
│   │       ├── money_parse.py         # Deterministic monetary amount parsing
│   │       ├── arabic_normalizer.py   # Arabic text normalization
│   │       ├── canonical_id.py        # UUID5 stable hash IDs for dedup
│   │       ├── finding_signature.py   # Finding signature for triage suppression
│   │       └── text_amount_scan.py    # Text-based amount scanning
│   ├── tests/
│   │   ├── test_amount_parsing_and_id.py
│   │   ├── test_amount_role_filtering.py
│   │   ├── test_canonical_id.py
│   │   ├── test_cross_checker_regression.py
│   │   ├── test_graph_canonical.py
│   │   ├── test_money_parse.py
│   │   └── test_reconciliation_payload.py
│   ├── requirements.txt
│   ├── Dockerfile
│   └── .env.example
├── frontend/
...
│   │   │   └── audit/[id]/
│   │   │       ├── layout.tsx        # Audit workspace layout
│   │   │       ├── loading.tsx       # Loading skeleton
│   │   │       └── page.tsx          # Workspace: findings, report, reconciliation, chat
│   │   ├── components/
│   │   │   ├── FileUpload.tsx        # Drag-and-drop PDF upload
│   │   │   ├── ReasoningPanel.tsx    # Live agent step feed (SSE)
│   │   │   ├── FindingsTable.tsx     # Findings with severity badges
│   │   │   ├── FindingDetailSheet.tsx
│   │   │   ├── AuditReport.tsx       # Full report, RTL-aware, JSON export
│   │   │   ├── ReconciliationPanel.tsx # Contract/invoice/bank variance card
│   │   │   └── ChatPanel.tsx         # Conversational Q&A panel
│   │   ├── hooks/
│   │   │   └── useAuditStream.ts     # SSE client hook
│   │   ├── lib/
│   │   │   ├── api.ts                # API client
